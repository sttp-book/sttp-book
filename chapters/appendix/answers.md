...
```java
public class SumOfIntegersPart1Test {

    private SumOfIntegers sumOfIntegers = new SumOfIntegers();

    @Property
    void pass(@ForAll @IntRange(min = 1, max = 99) int x, @ForAll @IntRange(min = 1, max = 99) int y){
        assertEquals(sumOfIntegers.sumPart1(x,y), x+y);
    }

    @Property
    void invalidX(@ForAll("invalidRange") int x, @ForAll @IntRange(min = 1, max = 99) int y){
        assertThrows(IllegalArgumentException.class, () -> sumOfIntegers.sumPart1(x,y));
    }

    @Property
    void invalidY(@ForAll @IntRange(min = 1, max = 99) int x, @ForAll("invalidRange") int y){
        assertThrows(IllegalArgumentException.class, () -> sumOfIntegers.sumPart1(x,y));
    }

    @Property
    void invalidBoth(@ForAll("invalidRange") int x, @ForAll("invalidRange") int y){
        assertThrows(IllegalArgumentException.class, () -> sumOfIntegers.sumPart1(x,y));
    }
...
4: Passing Concepts
```java
public class PassingConceptsTest {

    private PassingConcepts passingConcepts = new PassingConcepts();

    @Property
    void fail(@ForAll @FloatRange(min = 1f, max = 5f, maxIncluded = false) float x){
        assertEquals(passingConcepts.grade(x),'F');
    }

    @Property
    void passE(@ForAll @FloatRange(min = 5f, max = 6f, maxIncluded = false) float x){
        assertEquals(passingConcepts.grade(x),'E');
    }

    @Property
    void passD(@ForAll @FloatRange(min = 6f, max = 7f, maxIncluded = false) float x){
        assertEquals(passingConcepts.grade(x),'D');
    }

    @Property
    void passC(@ForAll @FloatRange(min = 7f, max = 8f, maxIncluded = false) float x){
        assertEquals(passingConcepts.grade(x),'C');
    }

    @Property
    void passB(@ForAll @FloatRange(min = 8f, max = 9f, maxIncluded = false) float x){
        assertEquals(passingConcepts.grade(x),'B');
    }

    @Property
    void passA(@ForAll @FloatRange(min = 9f, max = 10f, maxIncluded = true) float x){
        assertEquals(passingConcepts.grade(x),'A');
    }

    @Property
    void invalid(@ForAll("invalidRange") float x){
        assertThrows(IllegalArgumentException.class, () -> passingConcepts.grade(x));
    }
...
class TaxIncomeTest {
    ...
    void tax22100max(@ForAll @DoubleRange(min = 0, max = 22100, maxIncluded = false) double income){
        assertEquals(taxIncome.tax(income),0.15*income, Math.ulp(income));
    }

    @Property
    void tax53500max(@ForAll @DoubleRange(min = 22100, max = 53500, maxIncluded = false) double income){
        assertEquals(taxIncome.tax(income),3315 + 0.28 * (income - 22100), Math.ulp(income));
    }

    @Property
    void tax115000max(@ForAll @DoubleRange(min = 53500, max = 115000, maxIncluded = false) double income){
        assertEquals(taxIncome.tax(income),12107 + 0.31 * (income - 53500), Math.ulp(income));
    }

    @Property
    void tax250000max(@ForAll @DoubleRange(min = 115000, max = 250000, maxIncluded = false) double income){
        assertEquals(taxIncome.tax(income),31172 + 0.36 * (income - 115000), Math.ulp(income));
    }

    @Property
    void tax250000min(@ForAll @DoubleRange(min = 250000) double income){
        assertEquals(taxIncome.tax(income),79772 + 0.396 * (income - 250000), Math.ulp(income));
    }

    @Property
    void invalid(@ForAll("negative") double income){
        assertThrows(IllegalArgumentException.class, () -> taxIncome.tax(income));
    }
